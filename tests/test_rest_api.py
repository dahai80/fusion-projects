...
async def test_project_lifecycle(client):
    ...
    assert (await client.get(f"/api/v1/projects/{pid}")).json()["name"] == "A"
    assert len((await client.get("/api/v1/projects")).json()) == 1
    assert (await client.patch(f"/api/v1/projects/{pid}", json={"name": "A2"})).json()["name"] == "A2"
    assert (await client.post(f"/api/v1/projects/{pid}/star")).json()["is_starred"] is True

    assert (await client.delete(f"/api/v1/projects/{pid}")).status_code == 409
...
async def test_instruction_flow(client):
    pid = (await client.post("/api/v1/projects", json={"name": "B"})).json()["id"]

    assert (await client.get(f"/api/v1/projects/{pid}/instructions")).json()["content"] == ""
    assert (
        await client.put(f"/api/v1/projects/{pid}/instructions", json={"content": "hi"})
    ).json()["content"] == "hi"
    await client.put(f"/api/v1/projects/{pid}/instructions", json={"content": "hi2"})
    assert len(
        (await client.get(f"/api/v1/projects/{pid}/instructions/snapshots")).json()
    ) == 1
    assert (
        await client.delete(f"/api/v1/projects/{pid}/instructions")
    ).status_code == 204
    assert (
        await client.get(f"/api/v1/projects/{pid}/instructions")
    ).json()["content"] == ""
